...
psql -U "postgres" -d "clarity" -c "UPDATE "$NAME" x SET geom=ST_Multi(ST_CollectionExtract(ST_MakeValid( ST_Difference(x.geom, b.geom)),3)) FROM "$CITY"_built_up b WHERE ST_Contains(b.geom, x.geom) OR ST_Overlaps(x.geom, b.geom);"
echo "...removing built open spaces intersections..."
psql -U "postgres" -d "clarity" -c "UPDATE "$NAME" x SET geom=ST_Multi(ST_CollectionExtract(ST_MakeValid( ST_Difference(x.geom, b.geom)),3)) FROM "$CITY"_built_open_spaces b WHERE ST_Contains(b.geom, x.geom) OR ST_Overlaps(x.geom, b.geom);"
