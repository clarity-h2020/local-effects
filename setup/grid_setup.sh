...
then
	...
	psql -U "postgres" -d "clarity" -c "CREATE TABLE public.land_use_grid(id serial NOT NULL,cell integer NOT NULL REFERENCES laea_etrs_500m (gid),city integer NOT NULL REFERENCES city (id),water real default 0,roads real default 0,railways real default 0,trees real default 0,vegetation real default 0,agricultural_areas real default 0,built_up real default 0,built_open_spaces real default 0,dense_urban_fabric real default 0,medium_urban_fabric real default 0,low_urban_fabric real default 0,public_military_industrial real default 0,streams real default 0, basins real default 0, adaptation varchar(32) default null,UNIQUE(cell,adaptation), CONSTRAINT land_use_grid_pkey PRIMARY KEY (id));"
else
