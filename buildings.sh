...
######ADD height
######SOLO FUNCIONA CON LOS DATOS DE NAPOLES EN BBDD, HAY QUE CAMBIARLO PARA OTRAS CIUDADES EUROPEAS!!!!
psql -U "postgres" -d "clarity" -c "ALTER TABLE public.\""$NAME"\" ADD COLUMN height real DEFAULT null;"
psql -U "postgres" -d "clarity" -c "UPDATE public.\""$NAME"\" SET height = sub.height FROM (SELECT b.gid, AVG(CAST(h.altezza as real)) as height FROM buildings b, naples_height h WHERE ST_Intersects( b.geom , h.geom ) GROUP BY b.gid) sub WHERE buildings.gid=sub.gid;"
