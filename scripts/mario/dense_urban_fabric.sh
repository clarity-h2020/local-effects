...
#ogr2ogr -sql "SELECT area,perimeter FROM "$SHP" WHERE "$CODE"='11100' OR "$CODE"='11210'" $NAME $FILE
ogr2ogr -overwrite -sql "SELECT Shape_Area as area, Shape_Leng as perimeter FROM "$SHP" WHERE "$CODE"='11100' OR "$CODE"='11210'" $NAME $FILE
...
psql -U "postgres" -d "clarity" -c "UPDATE "$NAME" set geom=sq.geom from (SELECT ST_Multi(ST_CollectionExtract(ST_Difference( ST_Union(t.geom),ST_Multi(ST_Intersection(ST_Union(t.geom),ST_Union(r.geom))) ),3 ) )  as geom, m.gid FROM "$NAME" t, built_up r, land_use_grid g, laea_etrs_500m m WHERE g.city=1 AND g.cell=m.gid AND m.gid=t.cell AND r.cell=t.cell GROUP BY m.gid,m.geom) sq where cell=sq.gid;"
echo "...removing built open spaces intersections..."
psql -U "postgres" -d "clarity" -c "UPDATE "$NAME" set geom=sq.geom from (SELECT ST_Multi(ST_CollectionExtract(ST_Difference( ST_Union(t.geom),ST_Multi(ST_Intersection(ST_Union(t.geom),ST_Union(r.geom))) ),3 ) )  as geom, m.gid FROM "$NAME" t, built_open_spaces r, land_use_grid g, laea_etrs_500m m WHERE g.city=1 AND g.cell=m.gid AND m.gid=t.cell AND r.cell=t.cell GROUP BY m.gid,m.geom) sq where cell=sq.gid;"
...
#psql -U "postgres" -d "clarity" -c "INSERT INTO dense_urban_fabric (SELECT * FROM "$NAME");"
